...
//    id <- List("13-16", "12-11", "14-06", "15-06", "13-15")
    id <- List("15-06")
//    filename = s"***REMOVED***/Projects/14/ladder.sauer/indexed-data/collected/$id"
    filename = "***REMOVED***/Projects/14/ladder.sauer/indexed-data/collected/2/15-06"
  } yield new File(filename)

  println(s"Not found ==> ${targets.filterNot{_.exists}}")

  val target = new File("***REMOVED***/Projects/14/ladder.sauer/indexed-data/collected/2/15-06")
...
//  val ouch = for { ParsedMessage(_, t, m: PlayerExtInfo) <- yes } yield t -> m
  val ouch = yes
  val pw = new PrintWriter(new File("***REMOVED***/fuck.txt"))
...
//  val pw = new PrintWriter(new File("***REMOVED***/woot.xml"))
//  pw.println("<games>")
